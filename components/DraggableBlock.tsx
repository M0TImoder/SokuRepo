...
export const DraggableBlock: React.FC<DraggableBlockProps> = ({
  ...
  // 'quick' for inline input, 'full' for textarea
  ...
  // Sync temp content when block content updates externally
  useEffect(() => {
  ...
  // Click handler: Selects the block
  ...
  // Double click handler: Enters Edit Mode
  const handleBlockDoubleClick = (e: React.MouseEvent) => {
      if (isEditing) return;
      e.stopPropagation();
      // Input blocks default to quick edit (single line), Math blocks to full edit (textarea)
  ...
  const borderClass = isEditing 
    ...
    : isSelected
        ? 'border-solid border-blue-500 shadow-md z-40' // Selected state
        : 'border-dashed border-slate-400 hover:border-blue-400'; // Default state
  ...
  return (
    ...
    >
      {/* Toolbar - Visible if editing OR selected OR hovering */}
      <div 
      ...
      <div className="p-2 flex items-center justify-center h-full w-full min-h-[60px]">
        {isEditing ? (
            editMode === 'quick' ? (
                // Quick Edit Mode (Single Line Input)
                <input
                    ref={inputRef}
                    type="text"
